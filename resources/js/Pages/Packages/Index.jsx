...
function Index({ data }) {
  ...
  const columns = [
    ...
    { field: "duration", headerName: "Duration", width: 200, editable: true },
    {
      field: "description",
      ...
      renderCell: (params) => (
        <Typography
          onClick={() => handleDescriptionEdit(params.row.id, params.value)}
      ...
      renderCell: (params) => (
        <input
          ...
          onChange={(event) => {
            ...
            axios.put(`/packages/${params.row.id}`, { status: checked })
              .then((res) => {
    ...
    { field: "payment_method", headerName: "Payment Method", width: 200, editable: true },
  ];
  ...
  const submitDescriptionEdit = () => {
    axios.put(`/packages/${editDescriptionId}`, { description: editDescription })
      .then((res) => {
        if (res.data.check) {
  ...
  const submitPackage = () => {
    if (!name) {
      ...
      axios.post('/packages', { name, price, duration, description })
        .then((res) => {
          if (res.data.check == true) {
            setPackages(res.data.data);
        ...
        <div className="row">
          ...
          <Modal show={showDescriptionModal} onHide={() => setShowDescriptionModal(false)}>
            ...
            <Modal.Footer>
              ...
              <Button variant="primary" onClick={submitDescriptionEdit}>Update</Button>
            </Modal.Footer>
          </Modal>
